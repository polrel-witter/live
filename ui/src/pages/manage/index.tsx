      ...
      >
        {evt.details.latch === "over" &&
          (
            ...
            >
              this event is archived. changes cannot be made until the latch is
              changed to 'open' or 'closed'.
            </Button>
          )}
...
const Guests = ({ evt, profiles, open, setOpen, ...fns }: GuestProps) => {
  ...
  const StatusButton = (
    { status, patp }: { patp: Patp; status: EventStatus },
  ) => {
  ...
  useEffect(
    () => {
      ...
      if (statusFilter === "all") {
        ...
        setRecords(
          records.filter((record) => record[1].status === statusFilter),
        );
      }
    },
    [statusFilter],
...
export { ManageIndex, ManageParamsLoader };
